 ...
 * 
 */
public class TilePickBehavior extends PickMouseBehavior implements MouseBehaviorCallback {
	...
	private TileLogic tileLogic = null;

	public TilePickBehavior(BranchGroup root, Canvas3D canvas, Bounds bounds, int pickMode) {
	 ...
	 * @param bounds
	 */
	public TilePickBehavior(Canvas3D canvas, BranchGroup root, Bounds bounds, TileLogic tileLogic) {
		super(canvas, root, bounds);
	...
	public void updateScene(int xpos, int ypos) {
		...
		if (!mevent.isMetaDown() && !mevent.isAltDown()) {
			...
			if (pickInfo != null) {
				...
				ControlService.getService().setSelectionPosition(tileLogic.getPosition());
				ControlService.getService().setSelectionValue(tileLogic.getValue());
				ControlService.getService().setSelectionMade(true);
